  ...
  return (
    <div className="min-h-screen">
      ...
      <nav className="fixed top-0 w-full z-50 px-6 py-4">
        <div className="max-w-7xl mx-auto glass flex items-center justify-between !py-4 !px-8">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Incio', 'Funciones', 'Precios', 'Contacto'].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold text-[var(--text-secondary)] hover:text-[var(--primary)] transition-colors uppercase tracking-widest">{item}</a>
            ))}
            <button className="px-6 py-2.5 bg-[var(--primary)] text-white rounded-xl font-bold text-sm shadow-md hover:opacity-90 transition-all">
              EMPEZAR GRATIS
            </button>
          </div>

          <button className="md:hidden text-[var(--secondary)]" onClick={() => setIsMenuOpen(!isMenuOpen)}>
            {isMenuOpen ? <X /> : <Menu />}
          </button>
        </div>
      </nav>
      ...
      <section className="pt-40 pb-20 px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          <div className="flex-1 space-y-8 text-center lg:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[var(--primary-light)] text-[var(--primary)] rounded-full text-[10px] font-black uppercase tracking-[0.2em] animate-bounce">
              <CheckCircle2 size={12} />
              El POS #1 para Restaurantes Gastronómicos
            </div>
            <h1 className="text-6xl lg:text-7xl font-black text-[var(--secondary)] leading-[1.05] tracking-tight">
              Moderniza tu <span className="text-transparent bg-clip-text bg-gradient-to-r from-[var(--primary)] to-[var(--accent)]">Restaurante</span> ahora.
            </h1>
            <p className="text-xl text-[var(--text-secondary)] font-medium max-w-2xl mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
              <button className="btn-premium w-full sm:w-auto">Hacer Demo Gratis</button>
              <button className="px-8 py-4 rounded-2xl font-bold text-[var(--secondary)] border-2 border-gray-100 hover:bg-white hover:shadow-md transition-all w-full sm:w-auto uppercase tracking-wider text-sm">Ver Precios</button>
            </div>
          ...
          <div className="flex-1 relative">
            ...
            <div className="relative glass p-1 transition-transform duration-700 hover:rotate-1">
              <div className="bg-gradient-to-br from-[var(--secondary)] to-[var(--primary)] rounded-[28px] p-6 text-white space-y-8 shadow-2xl">
                <div className="flex items-center justify-between">
                  ...
                  <div className="px-3 py-1 bg-white/10 rounded-lg text-xs font-bold text-[var(--accent)]">+12.5%</div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center bg-white/10 p-3 rounded-2xl">
                    ...
                    <span className="text-xs font-black text-[var(--accent)]">COCINANDO</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-12">
          {[
            ...
            <div key={i} className="flex flex-col items-center text-center space-y-4">
              <div className="w-16 h-16 bg-[var(--background)] text-[var(--primary)] rounded-2xl flex items-center justify-center shadow-sm">
                <f.icon size={24} />
              </div>
              <h3 className="text-xl font-black text-[var(--secondary)]">{f.title}</h3>
              <p className="text-[var(--text-secondary)] font-medium text-sm leading-relaxed">{f.desc}</p>
            </div>
          ))}
        </div>
      </section>
