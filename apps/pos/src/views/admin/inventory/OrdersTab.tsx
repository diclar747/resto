        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4">
                {orders?.map((order: any) => (
                    <div key={order.id} className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-50 flex flex-col lg:flex-row lg:items-center justify-between gap-6 group">
                        ...
                        <div className="flex flex-wrap items-center gap-8">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <p className="text-[9px] font-black text-gray-400 uppercase">Solicitado</p>
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="p-3 bg-gray-50 hover:bg-gray-100 text-gray-400 hover:text-secondary rounded-2xl transition-all"
                            >
                                <Eye size={18} />
                            </button>
                            {order.status === 'PENDING' && (
                                <button
                                    ...
                                    className="px-6 py-3 bg-emerald-50 text-emerald-600 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all shadow-sm"
                                >
                                    Recibir Mercadería
                                </button>
                            )}
            ...
            {showForm && (
                <div className="fixed inset-0 bg-secondary/80 backdrop-blur-sm flex items-center justify-center z-[130] p-4">
                    <div className="bg-white rounded-[40px] p-10 w-full max-w-2xl shadow-2xl border border-white max-h-[90vh] overflow-y-auto no-scrollbar">
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    {orderForm.items.map((item, i) => (
                                        <div key={i} className="flex gap-3 bg-gray-50 p-4 rounded-[24px]">
                                            <select
                                                className="flex-1 bg-white px-4 py-3 rounded-xl text-xs font-bold outline-none border-none"
                                            ...
                                            <input
                                                type="number"
                                                placeholder="Cant"
                                                className="w-24 bg-white px-4 py-3 rounded-xl text-xs font-bold outline-none border-none"
                                                value={item.quantity}
                                                onChange={(e) => {
                                                    const items = [...orderForm.items];
                        ...
                        <div className="flex gap-4 mt-10">
                            <button onClick={() => setShowForm(false)} className="flex-1 py-4 bg-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-gray-200">Cancelar</button>
                            <button
