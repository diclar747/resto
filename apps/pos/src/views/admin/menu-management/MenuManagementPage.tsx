    ...
    <div className="p-8 space-y-8 bg-[#F4F7FE] min-h-screen">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/50 backdrop-blur-md p-6 rounded-[32px] border border-white shadow-xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-white text-secondary border border-gray-200 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-gray-50 transition-all shadow-sm"
          >
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredCategories?.map((category: any) => {
          ...
          return (
            <div key={category.id} className={`bg-white rounded-[40px] border border-white shadow-xl transition-all duration-500 overflow-hidden ${isExpanded ? 'ring-2 ring-primary/20' : ''}`}>
              {/* Category Header */}
              <div
                className={`p-6 flex items-center justify-between cursor-pointer transition-colors ${isExpanded ? 'bg-primary/5' : 'hover:bg-gray-50'}`}
                onClick={() => setExpandedCategory(isExpanded ? null : category.id)}
              >
                <div className="flex items-center gap-6">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg shadow-inner ${isExpanded ? 'bg-primary text-white' : 'bg-[#F4F7FE] text-secondary border border-white'}`}>
                    {category.name[0]}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="hidden md:flex -space-x-3">
                    {catProducts.slice(0, 4).map((p: any) => (
                      <div key={p.id} className="w-8 h-8 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center overflow-hidden">
                        {p.imageUrl ? <img src={p.imageUrl} alt="" className="w-full h-full object-cover" /> : <Layers size={12} className="text-gray-400" />}
                      </div>
                    ))}
                ...
                <div className="p-8 border-t border-gray-100 bg-gray-50/30">
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {catProducts.map((product: any) => (
                      <div key={product.id} className="bg-white rounded-[32px] p-6 shadow-md border border-white group relative">
                        <div className="flex items-start justify-between mb-4">
                          <div className="w-16 h-16 rounded-2xl bg-[#F4F7FE] border-2 border-white shadow-inner flex items-center justify-center overflow-hidden">
                        ...
                        <div className="space-y-2 mb-6">
                          {product.variants?.map((v: any) => (
                            <div key={v.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0 text-[11px] font-black uppercase tracking-wider">
                    ...
                    <button
                      ...
                      className="border-4 border-dashed border-gray-100 bg-gray-50/50 rounded-[32px] p-8 flex flex-col items-center justify-center text-gray-400 hover:border-primary/20 hover:bg-primary/5 hover:text-primary transition-all group"
                    >
      ...
      {showProductForm && (
        <div className="fixed inset-0 bg-secondary/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-2xl shadow-2xl border border-white relative max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="mb-10 flex items-center justify-between">
              ...
              <button onClick={() => setShowProductForm(false)} className="p-3 bg-gray-50 hover:bg-gray-100 rounded-2xl transition-all">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <input
                    className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                ...
                <div>
                  ...
                  <select
                    className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all appearance-none"
                ...
                <div>
                  ...
                  <textarea
                    className="w-full px-6 py-4 bg-gray-50 border-none rounded-2xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none transition-all min-h-[120px] resize-none"
                ...
                <div>
                  ...
                  <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
                    {productForm.variants.map((v, i) => (
                      <div key={i} className="bg-[#F4F7FE] p-4 rounded-3xl border border-white space-y-3 relative group/var">
                        <button
                        ...
                        <input
                          className="w-full px-4 py-2 bg-white rounded-xl text-xs font-black uppercase tracking-tight outline-none border border-transparent focus:border-primary/20"
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="relative">
                            ...
                            <input
                              className="w-full pl-8 pr-4 py-2 bg-white rounded-xl text-xs font-black outline-none border border-transparent focus:border-primary/20"
                          ...
                          <div className="relative">
                            ...
                            <input
                              className="w-full pl-8 pr-4 py-2 bg-white/50 rounded-xl text-xs font-black outline-none border border-transparent focus:border-primary/20"
                              placeholder="Costo"
                              type="number"
                              value={v.cost}
                              onChange={(e) => {
            ...
            <div className="mt-12 flex gap-6">
              <button
                ...
                className="flex-1 py-4 bg-gray-100 text-secondary rounded-2xl text-xs font-black uppercase tracking-widest transition-all hover:bg-gray-200"
              >
                Descartar Cambios
              </button>
              <button
        ...
        <div className="fixed inset-0 bg-secondary/80 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <div className="bg-white rounded-[40px] p-10 w-full max-w-md shadow-2xl border border-white">
            ...
            <div className="flex gap-4 mt-10">
              <button onClick={() => setShowCategoryForm(false)} className="flex-1 py-4 bg-gray-100 rounded-2xl text-[10px] font-black uppercase tracking-widest">Cerrar</button>
              <button
