...
const statusConfig: Record<string, { icon: any; label: string; color: string; bgColor: string }> = {
  ...
  sent: { icon: Package, label: 'Recibido', color: 'text-primary', bgColor: 'bg-primary/10' },
  preparing: { icon: ChefHat, label: 'En Cocina', color: 'text-orange-500', bgColor: 'bg-orange-50' },
  ready: { icon: Utensils, label: 'Listo p/ Servir', color: 'text-success', bgColor: 'bg-success/10' },
  delivered: { icon: CheckCircle, label: 'Entregado', color: 'text-secondary', bgColor: 'bg-gray-100' },
};
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="sticky top-0 z-30 p-4">
        <div className="bg-white/70 backdrop-blur-xl rounded-[28px] p-5 border border-white/40 shadow-xl flex items-center gap-4 !p-5 shadow-xl">
          <button
            onClick={() => navigate(`/mesa/${tableCode}`)}
            className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-secondary border border-gray-100 shadow-sm active:scale-95 transition-all"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Seguimiento</p>
            <h1 className="text-lg font-black text-secondary tracking-tight">Tu Pedido</h1>
          </div>
        </div>
      </div>
      ...
      <div className="px-5 space-y-6 pt-2">
        {isLoading && (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Sincronizando...</p>
          </div>
        )}

        {orders?.map((order: any) => (
          <div key={order.id} className="bg-white rounded-[28px] p-5 shadow-card border-none !p-6 space-y-6 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Orden #{order.orderNumber}</p>
              ...
              <div className="text-right">
                <p className="text-[10px] font-black text-text-secondary uppercase tracking-widest">Total</p>
                <p className="text-xl font-black text-primary tracking-tight">${Number(order.total).toLocaleString('es-AR')}</p>
              </div>
            </div>
        ...
        {orders?.length === 0 && !isLoading && (
          <div className="text-center py-20 px-10">
            ...
            <h3 className="text-xl font-black text-secondary tracking-tight mb-2">Sin Pedidos Activos</h3>
            ...
            <button
              onClick={() => navigate(`/mesa/${tableCode}`)}
              className="w-full py-4 rounded-2xl font-black bg-primary text-white shadow-[0px_8px_20px_rgba(0, 74, 173, 0.3)] active:scale-95 transition-all uppercase tracking-widest text-sm flex items-center justify-center gap-3"
            >
              IR AL MENÚ
            </button>
          </div>
      ...
      {!isLoading && orders?.length > 0 && (
        <div className="px-8 mt-10">
          <div className="bg-primary/5 p-6 rounded-[32px] border border-primary/10 border-dashed">
            <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-2 text-center">Tip de Experiencia</p>
            <p className="text-[11px] font-medium text-text-secondary text-center leading-relaxed">
